...
function isTwoColumnMode(): boolean {
    return currentWidth <= 140; // Threshold for 3-tab width mode (shows hamburger menu)
}
...
function renderSectionHeader(sectionName: string, isEmpty: boolean = false) {
    ...
    if (isNarrow) {
        ...
        // Wide mode: Show all icons normally
        renderWideSectionHeader(headerElement, sectionName, isEmpty);
    }
...
// Function to create section options dropdown for narrow mode
function createSectionOptionsDropdown(headerElement: HTMLElement, sectionName: string): HTMLElement {
    ...
    const options = [
        ...
        { icon: '✏️', label: 'Edit name', action: 'edit' },
        { icon: '×', label: 'Delete section', action: 'delete' }
    ];

    options.forEach(option => {
        const optionItem = document.createElement('div');
        optionItem.className = 'section-option-item';
        ...
        optionItem.addEventListener('mouseenter', () => {
            optionItem.style.backgroundColor = 'var(--tab-hover)';
        });
...
// Function to add common section event handlers
function addSectionEventHandlers(headerElement: HTMLElement, sectionName: string) {
...
// Function to render section header in wide mode (existing functionality)
function renderWideSectionHeader(headerElement: HTMLElement, sectionName: string, isEmpty: boolean) {
    const titleSpan = document.createElement('span');
    titleSpan.textContent = isEmpty ? `${sectionName} (Drop tabs here)` : sectionName;
    headerElement.appendChild(titleSpan);
    ...
    // Add drag handle for section reordering
